...
def separate_array_into_even_and_odd_numbers(array)
  array.group_by { |x| x.even? }.values.reverse
end
...
def average_of_array(array)
  (array.inject { |sum, element| sum + element }.to_f / array.size).ceil
end
...
def add_together_keys_and_values(hash)
  hash.flatten.inject(0){|sum,x| sum + x }
end
...
def titleize_a_string(string)
  string[0].capitalize + string[1..-1].split.each{ |x| x.length < 4 ? x : x.capitalize! }.join(" ")
end
